...
# BaseNetwork에는 weight init이 들어가 있다.
...
class Generator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        nf = opt.ngf # 64
        self.semantic_nc = opt.label_nc + (1 if self.opt.contain_dontcare_label else 0)
        ...
        if opt.use_vae:
            ...
            self.fc = nn.Conv2d(self.semantic_nc, 16 * nf, 3, padding=1)
    ...
    def forward(self, input, z=None):
        ...
        if self.opt.use_vae:
            ...
            # print(x.shape) # torch.Size([1, 3, 16, 16])
            x = self.fc(x)
...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt):
        ...
        self.opt = opt
        self.semantic_nc = self.opt.label_nc + (1 if self.opt.contain_dontcare_label else 0)
...
class NLayerDiscriminator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        nf = self.opt.ndf
        input_nc = self.compute_D_input_nc(self.opt)
